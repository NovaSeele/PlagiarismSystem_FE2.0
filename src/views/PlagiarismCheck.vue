<template>
  <div class="p-6 max-w-6xl mx-auto space-y-8">
    ...
    <div class="bg-white rounded-xl shadow p-4">
      ...
      <ul class="space-y-2">
        <li v-for="(categories, file) in fileCategories" :key="file" class="bg-gray-50 p-3 rounded-lg">
          <p class="font-medium text-gray-900">{{ file }}</p>
          <div class="flex flex-wrap mt-1 gap-2">
            <span v-for="category in categories" :key="category" class="bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded">
              {{ category }}
            </span>
          </div>
        </li>
    ...
    <div class="bg-white rounded-xl shadow p-4">
      ...
      <div class="mb-4">
        <label for="pairSelect" class="block text-sm font-medium text-gray-600 mb-1">Select File Pair</label>
      ...
      <div v-if="selectedPair" class="border-t pt-4 space-y-2">
        <p><span class="font-medium text-gray-700">Similarity:</span> {{ detailedReport[selectedPair].similarity_percentage }}</p>
        <p><span class="font-medium text-gray-700">Raw Score:</span> {{ detailedReport[selectedPair].raw_score.toFixed(4) }}</p>
        <div>
          ...
          <ul class="list-disc list-inside text-gray-600">
            ...
            <li>Transformer: {{ detailedReport[selectedPair].components.transformer.toFixed(4) }}</li>
          </ul>
        </div>
      </div>
    </div>
