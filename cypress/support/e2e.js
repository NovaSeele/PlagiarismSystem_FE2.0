// ***********************************************************
// This example support/e2e.js is processed and
// loaded automatically before your test files.
//
// This is a great place to put global configuration and
// behavior that modifies Cypress.
//
// You can change the location of this file or turn off
// automatically serving support files with the
// 'supportFile' configuration option.
//
// You can read more here:
// https://on.cypress.io/configuration
// ***********************************************************

// Import commands.js using ES2015 syntax:
import './commands'

// Alternatively you can use CommonJS syntax:
// require('./commands')

// Hide fetch/XHR requests from command log
if (Cypress.config('hideXHR')) {
  const app = window.top

  if (app && !app.document.head.querySelector('[data-hide-command-log-request]')) {
    const style = app.document.createElement('style')
    style.innerHTML = '.command-name-request, .command-name-xhr { display: none }'
    style.setAttribute('data-hide-command-log-request', '')

    app.document.head.appendChild(style)
  }
}

// Ignore uncaught exceptions from the application
Cypress.on('uncaught:exception', (err, runnable) => {
  // returning false here prevents Cypress from failing the test
  return false
})

// Set viewport size for consistency
beforeEach(() => {
  // Thiết lập viewport cho tests
  cy.viewport(1280, 720)
})

// Add a command to easily check if an element exists
Cypress.Commands.add('exists', { prevSubject: true }, (subject) => {
  expect(subject.length).to.be.greaterThan(0)
  return subject
})

// Handle API errors with reasonable defaults
Cypress.on('fail', (err, runnable) => {
  // Handle common API errors gracefully
  if (err.message.includes('API') || err.message.includes('XHR')) {
    console.warn('API error occurred:', err.message)
    return false
  }

  throw err
})
